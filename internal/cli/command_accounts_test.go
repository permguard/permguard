...
func TestCreateCommandForAccounts(t *testing.T) {
	...
	assert.NoError(err)
	output := buf.String()
	assert.Contains(output, "The official PermGuard CLI")
	assert.Contains(output, "Copyright © 2022 Nitro Agility S.r.l.")
	assert.Contains(output, "This command manages accounts.")
}
