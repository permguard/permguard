...
func TestDeleteCommandForIdentitySourcesDelete(t *testing.T) {
	args := []string{"-h"}
	outputs := []string{"The official Permguard Command Line Interface", "Copyright © 2022 Nitro Agility S.r.l.", "This command deletes a remote identity source."}
	aztestutils.BaseCommandTest(t, createCommandForIdentitySourceDelete, args, false, outputs)
}

// TestCliIdentitySourcesDeleteWithError tests the command for creating a identitysource with an error.
func TestCliIdentitySourcesDeleteWithError(t *testing.T) {
	tests := []struct {
		OutputType string
		HasError   bool
	}{
	...
	for _, test := range tests {
		args := []string{"identitysources", "delete", "--identitysource-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("DeleteIdentitySource", mock.Anything, mock.Anything).Return(nil, azerrors.ErrClientParameter)
...
// TestCliIdentitySourcesDeleteWithSuccess tests the command for creating a identitysource with an error.
func TestCliIdentitySourcesDeleteWithSuccess(t *testing.T) {
	tests := []string{
		"terminal",
		"json",
	}
	for _, outputType := range tests {
		args := []string{"identitysources", "delete", "--identitysource-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", outputType}
		outputs := []string{""}
		...
		zapClient.On("DeleteIdentitySource", mock.Anything, mock.Anything).Return(identitysource, nil)
		...
		if outputType == "terminal" {
			...
			outputPrinter["identity_sources"] = []*azmodelszap.IdentitySource{identitysource}
		}
