...
func TestDeleteCommandForTenantsDelete(t *testing.T) {
	...
	testutils.BaseCommandTest(t, createCommandForTenantDelete, args, false, outputs)
}
...
func TestCliTenantsDeleteWithError(t *testing.T) {
	...
	for _, test := range tests {
		args := []string{"tenants", "delete", "--tenant-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("DeleteTenant", mock.Anything, mock.Anything).Return(nil, cerrors.ErrClientParameter)
...
func TestCliTenantsDeleteWithSuccess(t *testing.T) {
	...
	for _, outputType := range tests {
		args := []string{"tenants", "delete", "--tenant-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", outputType}
		outputs := []string{""}
		...
		if outputType == "terminal" {
			...
			outputPrinter["tenant"] = []*zap.Tenant{tenant}
		}
