...
func TestUpdateCommandForIdentitiesUpdate(t *testing.T) {
	args := []string{"-h"}
	outputs := []string{"The official Permguard Command Line Interface", "Copyright © 2022 Nitro Agility S.r.l.", "This command updates a remote identity."}
	testutils.BaseCommandTest(t, createCommandForIdentityUpdate, args, false, outputs)
}
...
func TestCliIdentitiesUpdateWithError(t *testing.T) {
	...
	for _, test := range tests {
		args := []string{"identities", "update", "--identity-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("UpdateIdentity", mock.Anything).Return(nil, cerrors.ErrClientParameter)
...
func TestCliIdentitiesUpdateWithSuccess(t *testing.T) {
	...
	for _, outputType := range tests {
		args := []string{"identities", "update", "--identity-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", outputType}
		outputs := []string{""}
		...
		zapClient.On("UpdateIdentity", mock.Anything).Return(identity, nil)
