...
func TestListCommandForIdentitiesList(t *testing.T) {
	args := []string{"-h"}
	outputs := []string{"The official Permguard Command Line Interface", "Copyright © 2022 Nitro Agility S.r.l.", "This command lists all remote identities."}
	testutils.BaseCommandTest(t, createCommandForIdentityList, args, false, outputs)
}

// TestCliIdentitiesListWithError tests the command for creating an identity with an error.
func TestCliIdentitiesListWithError(t *testing.T) {
	tests := []struct {
		OutputType string
		HasError   bool
	}{
	...
	for _, test := range tests {
		args := []string{"identities", "list", "--identity-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("FetchIdentitiesBy", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, errors.New("operation error"))
...
// TestCliIdentitiesListWithSuccess tests the command for creating an identity with an error.
func TestCliIdentitiesListWithSuccess(t *testing.T) {
	tests := []string{
		"terminal",
		"json",
	}
	for _, outputType := range tests {
		args := []string{"identities", "list", "--identity-id", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", outputType}
		outputs := []string{""}
		...
		if outputType == "terminal" {
			...
			outputPrinter["identities"] = identities
		}
