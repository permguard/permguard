	...
	if ctx.IsTerminalOutput() {
		if len(authzResp.Evaluations) > 0 {
			...
			for _, eval := range authzResp.Evaluations {
				...
				if eval.Context != nil {
					if len(eval.Context.ID) > 0 {
						contextID = eval.Context.ID
					}
				}

				if len(eval.RequestID) > 0 {
					requestID = eval.RequestID
				}

				printer.Println(fmt.Sprintf("  - %s: %s, %s: %s, %s: %v",
					aziclicommon.KeywordText("Request ID"), aziclicommon.CreateText(requestID),
					aziclicommon.KeywordText("Context ID"), aziclicommon.CreateText(contextID),
					aziclicommon.KeywordText("Decision"), eval.Decision))

				if eval.Context != nil {
					if eval.Context.ReasonAdmin != nil {
						printer.Println(fmt.Sprintf("    - %s: Error: %s - %s ",
							aziclicommon.KeywordText("Reason Admin"),
							aziclicommon.IDText(eval.Context.ReasonAdmin.Code),
							eval.Context.ReasonAdmin.Message))
					}
					if eval.Context.ReasonUser != nil {
						printer.Println(fmt.Sprintf("    - %s: Error: %s - %s ",
							aziclicommon.KeywordText("Reason User"),
							aziclicommon.IDText(eval.Context.ReasonUser.Code),
