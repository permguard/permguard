...
// TestUpdateCommandForZonesUpdate tests the updateCommandForZonesUpdate function.
func TestUpdateCommandForZonesUpdate(t *testing.T) {
	args := []string{"-h"}
	outputs := []string{"The official Permguard Command Line Interface", "Copyright © 2022 Nitro Agility S.r.l.", "This command updates a remote zone."}
	aztestutils.BaseCommandTest(t, createCommandForZoneUpdate, args, false, outputs)
}

// TestCliZonesUpdateWithError tests the command for creating a zone with an error.
func TestCliZonesUpdateWithError(t *testing.T) {
	tests := []struct {
		OutputType string
		HasError   bool
	}{
	...
	for _, test := range tests {
		...
		cmd := createCommandForZoneUpdate(depsMocks, v)
		...
		zapClient.On("UpdateZone", mock.Anything).Return(nil, azerrors.ErrClientParameter)
...
// TestCliZonesUpdateWithSuccess tests the command for creating a zone with an error.
func TestCliZonesUpdateWithSuccess(t *testing.T) {
	tests := []string{
		"terminal",
		"json",
	}
	for _, outputType := range tests {
		...
		zapClient.On("UpdateZone", mock.Anything).Return(zone, nil)
