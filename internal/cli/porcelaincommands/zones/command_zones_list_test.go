...
func TestListCommandForZonesList(t *testing.T) {
	...
	aztestutils.BaseCommandTest(t, createCommandForZoneList, args, false, outputs)
}
...
func TestCliZonesListWithError(t *testing.T) {
	...
	for _, test := range tests {
		args := []string{"zones", "list", "--zoneid", "581616507495", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("FetchZonesBy", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, azerrors.ErrClientParameter)
...
func TestCliZonesListWithSuccess(t *testing.T) {
	...
	for _, outputType := range tests {
		args := []string{"zones", "list", "--zoneid", "581616507495", "--output", outputType}
		outputs := []string{""}
