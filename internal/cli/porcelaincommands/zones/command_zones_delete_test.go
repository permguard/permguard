...
func TestCliZonesDeleteWithError(t *testing.T) {
	...
	for _, test := range tests {
		args := []string{"--zone-id", "581616507495", "--output", test.OutputType}
		outputs := []string{""}
		...
		zapClient.On("DeleteZone", mock.Anything).Return(nil, errors.New("operation error"))
...
func TestCliZonesDeleteWithSuccess(t *testing.T) {
	...
	for _, outputType := range tests {
		args := []string{"--zone-id", "581616507495", "--output", outputType}
		outputs := []string{""}
