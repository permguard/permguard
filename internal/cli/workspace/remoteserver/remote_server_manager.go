...
func (m *RemoteServerManager) NOTPPush(server string, papPort int, accountID int64, repositoryID string, clientProvider NOTPClient) error {
	pppServer := fmt.Sprintf("%s:%d", server, papPort)
	papClient, err := aziclients.NewGrpcPAPClient(pppServer)
	if err != nil {
		return err
	...
	err = papClient.NOTPStream(hostHandler, accountID, repositoryID, notpstatemachines.PushFlowType)
	if err != nil {
		return err
	}
	return nil
