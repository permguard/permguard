...
func TestUpdateCommandForTenantsUpdate(t *testing.T) {
	args := []string{"-h"}
	outputs := []string{"The official PermGuard Command Line Interface", "Copyright © 2022 Nitro Agility S.r.l.", "This command updates a tenant."}
...
func TestCliTenantsUpdateWithError(t *testing.T) {
	...
	for _, outputType := range tests {
		args := []string{"tenants", "update", "--tenantid", "c3160a533ab24fbcb1eab7a09fd85f36", "--output", outputType}
		outputs := []string{""}
		...
		aapClient.On("UpdateTenant", mock.Anything).Return(nil, azerrors.ErrClientParameter)
...
func TestCliTenantsUpdateWithSuccess(t *testing.T) {
	...
	for _, outputType := range tests {
		...
		aapClient.On("UpdateTenant", mock.Anything).Return(tenant, nil)
