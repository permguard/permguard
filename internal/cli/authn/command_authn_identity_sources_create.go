...
func createCommandForIdentitySourceCreate(deps azcli.CliDependenciesProvider, v *viper.Viper) *cobra.Command {
	command := &cobra.Command{
		...
		Long: fmt.Sprintf(aziclicommon.CliLongTemplate, `This command creates an identity source.
...
Examples:
  ...
  permguard authn identitysources create --account 301990992055 --name permguard
	...
	v.BindPFlag(azconfigs.FlagName(commandNameForIdentitySourcesCreate, aziclicommon.FlagCommonName), command.Flags().Lookup(aziclicommon.FlagCommonName))
	return command
}
