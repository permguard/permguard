...
func newCliContext(cmd *cobra.Command, v *viper.Viper) (*CliCommandContext, error) {
	...
	if !azvalidators.IsValidPath(workDir) {
		return nil, azerrors.WrapSystemErrorWithMessage(azerrors.ErrCliDirectoryOperation, fmt.Sprintf("cli: %s is an invalid work directory", workDir))
	}
	ctx.workDir = workDir
	...
	if ctx.output != azcli.OutputTerminal && ctx.output != azcli.OutputJSON {
		return nil, azerrors.WrapSystemErrorWithMessage(azerrors.ErrCliDirectoryOperation, fmt.Sprintf("cli: %s is an invalid output", output))
	}
