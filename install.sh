...
# ----- release signature (cosign, keyless) -----
# The checksums file is signed at release time with cosign (GitHub OIDC,
# Fulcio certificate). Verifying it is what ties the download to the release
# pipeline rather than to whoever answers the download URL: the sha256 below
# only proves the archive matches a checksums file fetched from the same place.
#
# Verified when cosign is installed; required when PERMGUARD_VERIFY=signature.
...
verify_signature() {
  ...
  http_download "$certfile" "${BASE}/${CHECKSUM}.pem" || return 1
  cosign verify-blob \
...
if have cosign; then
  log "verifying release signature (cosign)…"
  verify_signature || die "the release signature on ${CHECKSUM} does not verify: refusing to install"
  log "release signature ok"
elif [ "${PERMGUARD_VERIFY:-}" = "signature" ]; then
  die "PERMGUARD_VERIFY=signature is set and cosign is not installed"
else
  log "note: cosign not found; skipping release-signature verification (sha256 only)."
  log "      install cosign, or set PERMGUARD_VERIFY=signature to make this fatal."
fi
