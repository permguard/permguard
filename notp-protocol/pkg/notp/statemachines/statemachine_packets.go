...
func createStatePacket(runtime *StateMachineRuntimeContext, messageCode uint16, messageValue uint64) (*aznotpsmpackets.StatePacket, *HandlerContext, error) {
	handlerCtx := &HandlerContext{
		flow:           runtime.FlowType(),
		bag:            runtime.bag,
		currentStateID: runtime.CurrentStateID(),
	}
	packet := &aznotpsmpackets.StatePacket{
...
func shouldHandlePacket(packet *aznotpsmpackets.StatePacket) bool {
	return packet.MessageCode != aznotpsmpackets.ActionResponseMessage && packet.MessageCode != aznotpsmpackets.StartFlowMessage
}

// createAndHandleStatePacket creates a state packet and handles it.
func createAndHandleStatePacket(runtime *StateMachineRuntimeContext, messageCode uint16, messageValue uint64, packetables []aznotppackets.Packetable) (*aznotpsmpackets.StatePacket, []aznotppackets.Packetable, bool, bool, error) {
	...
	var handledPacketables []aznotppackets.Packetable
	hasMore := false
...
func createAndHandleAndStreamStatePacket(runtime *StateMachineRuntimeContext, messageCode uint16, packetables []aznotppackets.Packetable) (aznotppackets.Packetable, bool, error) {
	messageValue := aznotppackets.CombineUint32toUint64(aznotpsmpackets.UnknownValue, aznotpsmpackets.UnknownValue)
...
func createAndHandleAndStreamStatePacketWithValue(runtime *StateMachineRuntimeContext, messageCode uint16, messageValue uint64, packetables []aznotppackets.Packetable) (aznotppackets.Packetable, bool, error) {
	var packet *aznotpsmpackets.StatePacket
	hasMore := true
	for hasMore {
		...
		packet = statePacket
		streamPacketables := append([]aznotppackets.Packetable{statePacket}, handledPacketable...)
...
func sendTermination(runtime *StateMachineRuntimeContext) error {
	statePacket := &aznotpsmpackets.StatePacket{
		MessageCode: aznotpsmpackets.TerminateMessage,
	}
...
func receiveAndHandleStatePacket(runtime *StateMachineRuntimeContext, expectedMessageCode uint16) (*aznotpsmpackets.StatePacket, []aznotppackets.Packetable, bool, error) {
	handlerCtx := &HandlerContext{
		flow:           runtime.FlowType(),
		bag:            runtime.bag,
		currentStateID: runtime.CurrentStateID(),
	...
	if statePacket.MessageCode == aznotpsmpackets.TerminateMessage {
