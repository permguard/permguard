...
func (t *TransportLayer) TransmitPacket(packetables []notppkts.Packetable) error {
	...
	writer, err := notppkts.NewPacketWriter(&packet)
	if err != nil {
		return err
	}
	writer.WriteProtocol(&notppkts.ProtocolPacket{Version: 1})
	...
	compressedData, err := data.CompressData(packet.Data)
	if err != nil {
		return err
	}
...
func (t *TransportLayer) ReceivePacket() ([]notppkts.Packetable, error) {
	...
	reader, err := notppkts.NewPacketReader(packet)
	if err != nil {
		return nil, err
	}
	...
	var state *notppkts.DataPacketState
	for {
		var data []byte
