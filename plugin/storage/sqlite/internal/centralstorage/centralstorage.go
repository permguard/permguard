...
type SqliteRepo interface {
	...
	UpsertApplication(tx *sql.Tx, isCreate bool, application *azifacade.Application) (*azifacade.Application, error)
	// DeleteApplication deletes an application.
	DeleteApplication(tx *sql.Tx, applicationID int64) (*azifacade.Application, error)
	// FetchApplication fetches an application.
	FetchApplications(db *sqlx.DB, page int32, pageSize int32, filterID *int64, filterName *string) ([]azifacade.Application, error)

	// UpsertIdentitySource creates or updates an identity source.
	UpsertIdentitySource(tx *sql.Tx, isCreate bool, identitySource *azifacade.IdentitySource) (*azifacade.IdentitySource, error)
	// DeleteIdentitySource deletes an identity source.
	DeleteIdentitySource(tx *sql.Tx, applicationID int64, identitySourceID string) (*azifacade.IdentitySource, error)
	// FetchIdentitySources fetches identity sources.
	FetchIdentitySources(db *sqlx.DB, page int32, pageSize int32, applicationID int64, filterID *string, filterName *string) ([]azifacade.IdentitySource, error)

	// UpsertIdentity creates or updates an identity.
	UpsertIdentity(tx *sql.Tx, isCreate bool, identity *azifacade.Identity) (*azifacade.Identity, error)
	// DeleteIdentity deletes an identity.
	DeleteIdentity(tx *sql.Tx, applicationID int64, identityID string) (*azifacade.Identity, error)
	// FetchIdentities fetches identities.
	FetchIdentities(db *sqlx.DB, page int32, pageSize int32, applicationID int64, filterID *string, filterName *string) ([]azifacade.Identity, error)

	// UpsertTenant creates or updates an tenant.
	UpsertTenant(tx *sql.Tx, isCreate bool, tenant *azifacade.Tenant) (*azifacade.Tenant, error)
	// DeleteTenant deletes an tenant.
	DeleteTenant(tx *sql.Tx, applicationID int64, tenantID string) (*azifacade.Tenant, error)
	// FetchTenant fetches an tenant.
	FetchTenants(db *sqlx.DB, page int32, pageSize int32, applicationID int64, filterID *string, filterName *string) ([]azifacade.Tenant, error)

	// UpsertLedger creates or updates a ledger.
	UpsertLedger(tx *sql.Tx, isCreate bool, ledger *azifacade.Ledger) (*azifacade.Ledger, error)
	// DeleteLedger deletes a ledger.
	DeleteLedger(tx *sql.Tx, applicationID int64, ledgerID string) (*azifacade.Ledger, error)
	// FetchLedgers fetches ledgers.
	FetchLedgers(db *sqlx.DB, page int32, pageSize int32, applicationID int64, filterID *string, filterName *string) ([]azifacade.Ledger, error)
	...
	UpsertKeyValue(tx *sql.Tx, keyValue *azifacade.KeyValue) (*azifacade.KeyValue, error)
	// DeleteKeyValue deletes a key value.
	GetKeyValue(db *sqlx.DB, key string) (*azifacade.KeyValue, error)
}
...
func (s SqliteExec) Connect(ctx *azstorage.StorageContext, sqliteConnector azidb.SQLiteConnector) (*sqlx.DB, error) {
	...
	if err != nil {
		return nil, azifacade.WrapSqlite3Error("cannot connect to sqlite", err)
	}
	return db, nil
}
