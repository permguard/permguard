...
# Verified when cosign is on PATH; required when $env:PERMGUARD_VERIFY = "signature".
# The sha256 below only proves the archive matches a checksums file fetched from
# the same place; the cosign signature is what ties it to the release pipeline.
...
if ($cosign) {
  ...
  throw "PERMGUARD_VERIFY=signature is set and cosign is not installed"
} else {
  Write-Host "[permguard-install] Note: cosign not found; skipping release-signature verification (sha256 only)." -ForegroundColor Yellow
}
