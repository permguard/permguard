...
`tests/`, `.git/`); it reads like `.gitignore` — a name matches at any depth, a path with a `/`
is a prefix from the root — and it is a starting point, not a rule. What an operating system
drops on its own (`.DS_Store`, `Thumbs.db`, `desktop.ini`) is never read and needs no entry.
`checkout` of another ledger refuses while
