...
`tests/`, `.git/`, `.DS_Store`); it reads like `.gitignore` — a name matches at any depth, a path
with a `/` is a prefix from the root — and it is a starting point, not a rule. `checkout` of another ledger refuses while
